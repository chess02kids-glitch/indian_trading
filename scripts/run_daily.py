...
from execution.idempotency import IdempotencyRegistry  # noqa: E402
from execution.paper import PaperBroker, PaperBrokerConfig  # noqa: E402
from execution.service import ExecutionService  # noqa: E402
from observability.alerts import AlertService  # noqa: E402
from observability.health import HealthService, SystemHealth  # noqa: E402
from orchestration.pipeline import DailyPipeline, ManualApprovalGate  # noqa: E402
from portfolio.construction import EqualWeightConstructor  # noqa: E402
from reconciliation.engine import ReconciliationEngine  # noqa: E402
from research.strategies import MomentumStrategy  # noqa: E402
from risk_kill import RiskGuard, RiskLimits  # noqa: E402
from store.sqlite import SQLiteStore  # noqa: E402

EXIT_CODES = {
    "completed": 0,
    "duplicate_run": 10,
    "halted_data_quality": 20,
    "halted_risk": 21,
    "awaiting_approval": 22,
    "locked_reconciliation": 23,
...
def _args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--data", required=True, type=Path, help="validated candidate CSV/Parquet input"
    )
    parser.add_argument(
        "--run-date", default=date.today().isoformat(), help="ISO trading date (UTC)"
    )
    parser.add_argument(
        "--state-db",
        type=Path,
        default=Path(os.getenv("QUANT_INDIA_PAPER_DB", "var/paper_daily.sqlite")),
    )
    parser.add_argument(
        "--status-file",
        type=Path,
        default=Path(
            os.getenv("QUANT_INDIA_PAPER_STATUS", "var/operational_status.json")
        ),
    )
    parser.add_argument(
        "--max-staleness-days",
        type=float,
        default=float(os.getenv("QUANT_INDIA_MAX_STALENESS_DAYS", "6")),
    )
    parser.add_argument(
        "--approved-by", help="explicit human approver identity; absent is NOT approved"
    )
...
def main() -> int:
    ...
    try:
        ...
        service = ExecutionService(
            broker=broker,
            order_repository=store.orders,
            position_repository=store.positions,
            risk_guard=guard,
        ...
        pipeline = DailyPipeline(
            ...
            risk_guard=guard,
            run_repository=store.runs,
            position_repository=store.positions,
            order_repository=store.orders,
            research_repository=store.research,
            reconciliation_repository=store.reconciliation,
            reconciliation_engine=ReconciliationEngine(guard),
            health_service=health,
            alert_service=AlertService(),
            approval_gate=ManualApprovalGate(),
            dataset_version="daily-local",
        ...
        status.update(
            {
                ...
                "mode": "PAPER",
                "approval_state": "APPROVED" if result.approved else "NOT_APPROVED",
