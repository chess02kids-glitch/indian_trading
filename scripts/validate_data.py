...
def validate():
    ...
    for req in reqs["datasets"]:
            ...
            for col, forbidden_val in req.get("must_not_contain", {}).items():
                if col in df.columns:
                    if (df[col] == forbidden_val).any():
                        print(
                            f"  [FAIL] File {file} contains forbidden value '{forbidden_val}' in column '{col}'"
                        )
                        all_passed = False
        ...
        if "min_rows" in req and total_rows < req["min_rows"]:
            print(
                f"  [FAIL] Total rows {total_rows} is less than required {req['min_rows']}"
            )
            all_passed = False
        ...
        if "min_date" in req:
            ...
            if actual_min > min_req:
                print(
                    f"  [FAIL] Minimum date found is {actual_min}, required at least {min_req}"
                )
                all_passed = False
