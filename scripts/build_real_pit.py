...
def build_pit():
    ...
    events_by_index = defaultdict(
        lambda: defaultdict(lambda: {"included": set(), "excluded": set()})
    )
    ...
    for file in json_files:
        ...
        for event in data.get("events", []):
            ...
            events_by_index[idx_name][date]["included"].update(
                event.get("included", [])
            )
            events_by_index[idx_name][date]["excluded"].update(
                event.get("excluded", [])
            )
    ...
    pd.DataFrame(recon_records).to_csv(
        "data/universe/reconstitution_events.csv", index=False
    )
