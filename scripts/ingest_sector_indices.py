...
def main():
    ...
    for csv_file in src_dir.glob("nifty*.csv"):
        ...
        try:
            ...
            # rename to canonical
            df.rename(
                columns=lambda x: header_map.get(str(x).strip().lower(), str(x)),
                inplace=True,
            )
            ...
            df["close"] = pd.to_numeric(
                df.get("Close", df.get("close")), errors="coerce"
            )
            df["volume"] = pd.to_numeric(
                df.get("Volume", df.get("volume")), errors="coerce"
            ).fillna(0)
            ...
            is_broad = bool(
                re.match(
                    r"^NIFTY_?(50|100|200|500|NEXT_50|MIDCAP|SMALLCAP|LARGEMIDCAP|MICROCAP|TOTAL_MARKET)",
                    name,
                )
            )
