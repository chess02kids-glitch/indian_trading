...
def main():
            ...
            records.append(
                {
                    ...
                    "avg_daily_traded_value": info.get("averageVolume10days", 0)
                    * info.get("currentPrice", 0)
                    if info.get("averageVolume10days") and info.get("currentPrice")
                    else None,
    ...
    print(
        f"Successfully wrote {len(sector_df)} records to sector_map.csv and float.csv"
    )
