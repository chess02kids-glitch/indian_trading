...
def calc_wma(series: pd.Series, period: int) -> pd.Series:
    ...
    if talib is not None and hasattr(talib, 'WMA'):
        return pd.Series(talib.WMA(series.values.astype(float), timeperiod=period), index=series.index)
...
def calc_macd(data: pd.DataFrame, fast: int = 12, slow: int = 26, signal: int = 9) -> Tuple[pd.Series, pd.Series, pd.Series]:
...
def calc_bollinger(data: pd.DataFrame, period: int = 20, std_dev: float = 2.0) -> Tuple[pd.Series, pd.Series, pd.Series]:
...
def calc_stochastic(data: pd.DataFrame, k_period: int = 14, d_period: int = 3) -> Tuple[pd.Series, pd.Series]:
...
def calc_adx(data: pd.DataFrame, period: int = 14) -> Tuple[pd.Series, pd.Series, pd.Series]:
    ...
    low = _squeeze(data, 'Low')
    close = _squeeze(data, 'Close')
    if talib is not None and hasattr(talib, 'ADX') and hasattr(talib, 'PLUS_DI') and hasattr(talib, 'MINUS_DI'):
        adx = pd.Series(talib.ADX(high.values.astype(float), low.values.astype(float), close.values.astype(float), timeperiod=period), index=data.index)
        pdi = pd.Series(talib.PLUS_DI(high.values.astype(float), low.values.astype(float), close.values.astype(float), timeperiod=period), index=data.index)
        mdi = pd.Series(talib.MINUS_DI(high.values.astype(float), low.values.astype(float), close.values.astype(float), timeperiod=period), index=data.index)
        return adx, pdi, mdi
...
def calc_cci(data: pd.DataFrame, period: int = 20) -> pd.Series:
    ...
    if talib is not None and hasattr(talib, 'CCI'):
        return pd.Series(talib.CCI(high.values.astype(float), low.values.astype(float), close.values.astype(float), timeperiod=period), index=data.index)
...
def calc_williams_r(data: pd.DataFrame, period: int = 14) -> pd.Series:
    ...
    if talib is not None and hasattr(talib, 'WILLR'):
        return pd.Series(talib.WILLR(high.values.astype(float), low.values.astype(float), close.values.astype(float), timeperiod=period), index=data.index)
...
def calc_donchian(data: pd.DataFrame, period: int = 20) -> Tuple[pd.Series, pd.Series, pd.Series]:
...
def calc_keltner(data: pd.DataFrame, ema_period: int = 20, atr_period: int = 10, multiplier: float = 2.0) -> Tuple[pd.Series, pd.Series, pd.Series]:
...
def calc_obv(data: pd.DataFrame) -> pd.Series:
    ...
    volume = _squeeze(data, 'Volume')
    diff = close.diff()
    sign = pd.Series(np.where(diff > 0, 1.0, np.where(diff < 0, -1.0, 0.0)), index=data.index)
...
def download_data(symbol: str, period: str = "60d", interval: str = "1h", prefer_local: bool = False) -> Optional[pd.DataFrame]:
    """
...
def get_nifty_data(symbol: str = "NIFTY_50", period: str = "60d") -> Optional[pd.DataFrame]:
...
def backtest_strategy(
    ...
    if getattr(strategy, 'requires_volume', False) and not is_volume_valid(df):
        return pd.DataFrame(), initial_capital, pd.Series([initial_capital] * len(df), index=df.index)
...
def score_trades(
    ...
    if equity_curve is not None and len(equity_curve) > 5:
        ...
        downside_vol = float(downside.std(ddof=1) * np.sqrt(periods_per_year) * 100.0) if len(downside) > 1 else 1e-10
...
def run_portfolio_backtest(
    ...
    for ts in all_timestamps:
        ...
        for sym, pos in list(open_positions.items()):
            ...
            if atr_stop_multiplier > 0 and low <= stop_price:
                ...
                all_trades.append({
                    ...
                    'Qty': pos['qty'],
                    'Net_PnL': round(net_pnl, 2),
                    'Return %': round((exit_price - pos['buy_price']) / pos['buy_price'] * 100.0, 2),
                    'Exit_Reason': 'STOP_LOSS'
                })
        ...
        if len(open_positions) < max_positions and cash > (total_equity * 0.05):
            for sym, df in processed.items():
                ...
                if qty >= 1:
                    stop_price = max(price - (atr * atr_stop_multiplier), 0.01) if atr_stop_multiplier > 0 else 0.0
    ...
    perf = score_trades(trades_df, initial_capital, strategy.name, getattr(strategy, 'group', 'Portfolio'), equity_curve=eq_series)

    return {
        'portfolio_metrics': perf,
        'trades': trades_df,
        'equity_curve': eq_series,
        'final_equity': round(float(eq_series.iloc[-1]) if not eq_series.empty else initial_capital, 2)
    }
...
def walk_forward_validate_strategy(
    ...
    is_trades, is_final, is_equity = backtest_strategy(
        train_df, strategy, atr_stop_multiplier=atr_stop_multiplier, brokerage_bps=cost_bps
    )
    is_metrics = score_trades(is_trades, 50000.0, strategy.name, strategy.group, equity_curve=is_equity, n_trials=n_trials)

    # Out-of-Sample Backtest
    oos_trades, oos_final, oos_equity = backtest_strategy(
        test_df, strategy, atr_stop_multiplier=atr_stop_multiplier, brokerage_bps=cost_bps
    )
    oos_metrics = score_trades(oos_trades, 50000.0, strategy.name, strategy.group, equity_curve=oos_equity, n_trials=n_trials)
...
def run_all_strategies(
    ...
    for i, strategy in enumerate(ALL_STRATEGIES):
        ...
        if getattr(strategy, 'requires_volume', False) and not is_volume_valid(df):
            ...
            results.append(perf)
            print(f"  [{i+1:02d}/30] {strategy.name:<40} | ⚠️ NO VOLUME DATA (Index Symbol)")
            continue

        trades_df, final_cap, equity_curve = backtest_strategy(
            df, strategy, initial_capital=capital, brokerage_bps=brokerage_bps, atr_stop_multiplier=atr_stop_multiplier
        )
        perf = score_trades(trades_df, capital, strategy.name, strategy.group, equity_curve=equity_curve, n_trials=len(ALL_STRATEGIES))
        results.append(perf)

        print(f"  [{i+1:02d}/30] {strategy.name:<40} | "
              f"{perf['Rating']:<15} | "
...
def run_idea_generation_pipeline(
    ...
    print(f"\n=======================================================")
    print(f"🚀 RUNNING TIER 1: CANDIDATE SCREENING ({len(strategies)} strategies × {len(symbols)} symbols)")
    print(f"=======================================================")
    ...
    print(f"\n=======================================================")
    print(f"🔬 RUNNING TIER 2: WALK-FORWARD VALIDATION ON TOP CANDIDATES")
    print(f"=======================================================")
    ...
    for _, row in top_screened.iterrows():
        ...
        print(f"  👉 [{sym}] {strat_name}: Gate = {wf_res['gate_decision']} "
...
def get_top_strategies(results_df: Optional[pd.DataFrame], top_n: int = 10) -> pd.DataFrame:
    ...
    if results_df is None or results_df.empty:
        return pd.DataFrame()
    return results_df.sort_values(by=['Score', 'Sharpe', 'Return %'], ascending=False).head(top_n)
...
def get_best_by_group(results_df: Optional[pd.DataFrame]) -> Dict[str, Any]:
    ...
    for group in results_df['Group'].unique():
        ...
        if not grp_df.empty:
            best[group] = grp_df.sort_values(by=['Score', 'Sharpe'], ascending=False).iloc[0].to_dict()
    return best
...
def generate_dashboard_chart(results_df: Optional[pd.DataFrame], symbol: str) -> Optional[str]:
    ...
    axes[0, 0].axvline(x=1.0, color='green', linestyle='--', linewidth=1, label='1.0 Target')
    axes[0, 0].axvline(x=0.0, color='red', linestyle='-', linewidth=1)
    axes[0, 0].set_title('Annualized Sharpe Ratio (Excess over 6% Rf)', fontweight='bold')
    ...
    axes[0, 1].set_title('Total Return % (Net of 15 bps Costs & ATR Stops)', fontweight='bold')
    axes[0, 1].tick_params(axis='y', labelsize=7)

    # 3. Max Drawdown %
    axes[1, 0].barh(names, results_df['Max Drawdown %'], color=colors)
    axes[1, 0].axvline(x=15.0, color='orange', linestyle='--', linewidth=1, label='15% Max DD Limit')
    ...
    axes[1, 1].axvline(x=6, color='red', linestyle='--', linewidth=1, label='Score ≥ 6 = Viable')
