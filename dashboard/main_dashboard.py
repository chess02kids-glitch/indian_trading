...
def live_trading_unlockable(strategy_id: str | None = None) -> tuple[bool, list[str]]:
    ...
    if strategy_id:
        ...
        for record in mlflow_data:
            if record.get('hypothesis_id') == strategy_id or record.get('strategy') == strategy_id:
...
def render_go_live_gate(strategy_id: str | None = None) -> bool:
    ...
    with col1:
        for check_name in [
            ...
            st.markdown(f"{status_icon} **{check_name}** :{status_color}[{'PASS' if passed else 'FAIL'}]")
    ...
    with col2:
        if unlockable:
            ...
            if st.button("🚀 Enable Live Trading", disabled=confirmation != strategy_id):
...
def render_command_center() -> None:
    ...
    if diagnostics:
        ...
        overall_color = "green" if overall == "passed" else "red" if overall == "failed" else "orange"
...
def render_research_pipeline() -> None:
    ...
    for record in mlflow_data:
        display_data.append({
            ...
            "Sharpe (cost-adjusted)": record.get("metrics", {}).get("sharpe_cost_adjusted", 0),
            "Deflated Sharpe": record.get("validation", {}).get("deflated_sharpe", {}).get("probability", 0),
            "CPCV": "PASS" if record.get("gate_result", {}).get("cpcv_pass") else "FAIL",
    ...
    selected_strategy = st.selectbox(
        "Select strategy to view gate status",
    ...
    for record in mlflow_data:
        if record.get("hypothesis_id") == selected_strategy or record.get("run_id") == selected_strategy:
...
def render_risk_kill_switch() -> None:
    ...
    with col1:
        ...
        if daily_loss_limit > 0:
            ...
            st.progress(ratio)
            st.caption(f"₹{daily_loss_used:,.2f} / ₹{daily_loss_limit:,.2f} ({ratio*100:.1f}%)")
        else:
...
def render_capital_ladder() -> None:
    ...
    live_enabled = render_go_live_gate(strategy_id if strategy_id else None)
    ...
    for criterion in criteria:
        ...
        st.markdown(f"{status} **{criterion}**: :{color}[{'PASS' if passed else 'FAIL'}]")
...
def render_reconciliation() -> None:
    ...
    with col1:
        ...
        if token_expiry != "unknown":
            expiry_time = datetime.fromisoformat(token_expiry) if isinstance(token_expiry, str) else token_expiry
            time_remaining = (expiry_time - datetime.now(timezone.utc)).total_seconds() / 3600
...
def render_alerts() -> None:
    ...
    if alerts:
        ...
        for alert in filtered_alerts:
            ...
            with st.expander(f":{severity_color}[{severity}] {alert.get('timestamp', 'Unknown')}"):
    ...
    if health.get("state") == "LOCKED":
        ...
        **Manual actions taken**: {}
        """
        st.markdown(incident_text.format(
        ...
        with st.form("incident_log"):
            ...
            if st.form_submit_button("Log Incident"):
                ...
                try:
                    ...
                    incident_path = Path(f"var/incidents/incident_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main() -> None:
    ...
    # Title
    st.markdown('<p class="main-header">Quant India - Dashboard & Control Plane (v1)</p>', 
                unsafe_allow_html=True)
    st.markdown("**Read-only from everything below** | **Writes only to**: pause/resume, promote/demote, manual kill switch")
    ...
    # Footer
    st.markdown("---")
    st.caption("Quant India - Dashboard v1 | Read-only operational view | Architecture: Dashboard reads from health_check.json, MLflow, risk_kill/, execution/, reconciliation/")
