...
from config.env_validator import validate_environment, ConfigurationError
from observability.alerts import AlertService, AlertSeverity
...
from models.domain import ReconciliationMismatch
...
def test_env_validator_credentials_in_paper():
    with patch.dict(os.environ, {"SYSTEM_MODE": "PAPER", "DATABASE_URL": "db", "UPSTOX_API_KEY": "secret"}, clear=True):
        with pytest.raises(ConfigurationError):
            validate_environment()
