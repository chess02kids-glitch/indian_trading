...
Tests risk controls, walk-forward validation, multi-symbol portfolio mode,
...
class TestStrategyPerformanceEngine:
    """Test suite covering all 6 architectural improvements & risk defenses."""
    ...
    def test_atr_stop_loss_trigger(self, sample_ohlcv_data):
        ...
        if not trades.empty:
            ...
            assert set(trades["Exit_Reason"].unique()).issubset({"STOP_LOSS", "SIGNAL_EXIT"})
