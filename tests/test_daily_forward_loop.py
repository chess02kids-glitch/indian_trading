...
def test_daily_runner_is_paper_only_and_has_deterministic_exit_codes() -> None:
    ...
    codes = next(
        ...
        if any(
            isinstance(target, ast.Name) and target.id == "EXIT_CODES"
    ...
    values = {
        ...
        for key, value in zip(codes.keys, codes.values, strict=True)
    }
    assert values == {
        "completed": 0,
        "duplicate_run": 10,
...
def test_daily_service_uses_only_the_canonical_paper_runner() -> None:
    service = Path("deploy/systemd/quant-india-daily.service").read_text(
        encoding="utf-8"
    )
...
def test_sqlite_claim_can_resume_only_approval_pending_run(tmp_path) -> None:
    from store.sqlite import SQLiteStore

    store = SQLiteStore(tmp_path / "state.sqlite")
    assert store.runs.claim_run("paper-2026-08-25")
    store.runs.save_run("paper-2026-08-25", "awaiting_approval")
    assert store.runs.claim_run("paper-2026-08-25", resume_awaiting_approval=True)
    store.runs.save_run("paper-2026-08-25", "completed")
    assert not store.runs.claim_run("paper-2026-08-25", resume_awaiting_approval=True)
