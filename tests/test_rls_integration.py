...
def test_rls_cross_tenant_isolation(db_conn):
    ...
    with db_conn.cursor() as cur:
        ...
        try:
            ...
            pytest.skip(f"Could not insert order (maybe missing auth user dependency): {e}")

        # Ensure User A can read it
        cur.execute("SELECT internal_order_id FROM public.orders WHERE internal_order_id = %s", (order_id,))
